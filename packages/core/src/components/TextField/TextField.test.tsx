...
describe('TextField', () => {
  ...
  describe('상태 prop 테스트', () => {
    ...
    it('readOnly 상태를 적용한다', () => {
      render(<TextField label="이름" readOnly value="홍길동" onChange={() => {}} />)
      const input = screen.getByLabelText('이름')
  ...
  describe('error prop 테스트', () => {
    ...
    it('error message를 표시한다', () => {
      render(<TextField label="이메일" error errorMessage="올바른 이메일을 입력하세요" />)
      expect(screen.getByText('올바른 이메일을 입력하세요')).toBeInTheDocument()
    })

    it('error가 false이면 error message를 표시하지 않는다', () => {
      render(<TextField label="이메일" errorMessage="올바른 이메일을 입력하세요" />)
  ...
  describe('fullWidth prop 테스트', () => {
    it('fullWidth가 true이면 전체 너비를 차지한다', () => {
      ...
      const wrapper = screen.getByLabelText('이름').parentElement?.parentElement
  ...
  describe('접근성 테스트', () => {
    ...
    it('errorMessage가 있을 때 aria-describedby가 설정된다', () => {
      ...
      expect(screen.getByText('올바른 이메일을 입력하세요')).toHaveAttribute('id', describedBy!)
    })
    ...
    it('helperText가 있을 때 aria-describedby가 설정된다', () => {
      ...
      const describedBy = input.getAttribute('aria-describedby')
      expect(describedBy).toBeTruthy()
      expect(screen.getByText('8자 이상 입력하세요')).toHaveAttribute('id', describedBy!)
    })
