...
describe('Switch', () => {
  ...
  describe('helperText prop 테스트', () => {
    it('helperText를 표시한다', () => {
      render(<Switch label="스위치" helperText="알림을 받으려면 활성화하세요" />)
      expect(screen.getByText('알림을 받으려면 활성화하세요')).toBeInTheDocument()
    })

    it('error가 있으면 helperText를 표시하지 않는다', () => {
      render(<Switch label="스위치" helperText="도움말" error errorMessage="에러" />)
  ...
  describe('접근성 테스트', () => {
    ...
    it('aria-describedby를 helperText와 연결한다', () => {
      ...
      expect(screen.getByText('도움말 텍스트')).toHaveAttribute('id', describedBy)
    })
    ...
    it('aria-describedby를 errorMessage와 연결한다', () => {
      ...
      const describedBy = switchElement.getAttribute('aria-describedby')
      expect(describedBy).toBeTruthy()
      expect(screen.getByText('에러 메시지')).toHaveAttribute('id', describedBy)
    })
    ...
    it('label과 input이 올바르게 연결된다', () => {
      ...
      expect(labelElement.closest('label')).toHaveAttribute('for', switchElement.id)
    })
  })
