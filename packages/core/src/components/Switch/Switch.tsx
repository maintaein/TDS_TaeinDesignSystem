...
export const Switch = forwardRef<HTMLInputElement, SwitchProps>(
  (
    ...
    if (process.env.NODE_ENV === 'development') {
      if (!label) {
        console.error('Switch: label prop은 필수입니다. 접근성을 위해 label을 제공하세요.')
      }
    }
    ...
    return (
      <div className={styles.wrapper}>
        ...
        <label htmlFor={switchId} className={styles.switchLabel}>
          ...
          {/* Switch Track */}
          <div className={clsx(styles.switchContainer, styles.size[size], error && styles.error)}>
            <input
              ref={ref}
              id={switchId}
              type="checkbox"
              ...
              className={clsx(styles.switchInput, error && styles.error, className)}
              {...rest}
            />
