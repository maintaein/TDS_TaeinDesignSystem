...
describe('TextArea', () => {
  ...
  describe('autoResize prop 테스트', () => {
    ...
    it('autoResize가 true일 때 텍스트 입력 시 높이가 조정된다', async () => {
      ...
      await userEvent.type(textarea, '첫 줄\n두 번째 줄\n세 번째 줄\n네 번째 줄')
  ...
  describe('상태 prop 테스트', () => {
    ...
    it('readOnly 상태를 적용한다', () => {
      render(<TextArea label="설명" readOnly value="읽기 전용" onChange={() => {}} />)
      const textarea = screen.getByLabelText('설명')
  ...
  describe('error prop 테스트', () => {
    ...
    it('error message를 표시한다', () => {
      render(<TextArea label="설명" error errorMessage="필수 입력 항목입니다" />)
  ...
  describe('helperText prop 테스트', () => {
    it('helperText를 표시한다', () => {
      render(<TextArea label="설명" helperText="최대 500자까지 입력 가능합니다" />)
    ...
    it('error가 true이면 helperText 대신 errorMessage를 표시한다', () => {
      ...
      expect(screen.queryByText('최대 500자까지 입력 가능합니다')).not.toBeInTheDocument()
    })
  })
  ...
  describe('fullWidth prop 테스트', () => {
    it('fullWidth가 true이면 전체 너비를 차지한다', () => {
      ...
      const wrapper = screen.getByLabelText('설명').parentElement?.parentElement
  ...
  describe('접근성 테스트', () => {
    ...
    it('errorMessage가 있을 때 aria-describedby가 설정된다', () => {
      ...
      expect(screen.getByText('필수 입력 항목입니다')).toHaveAttribute('id', describedBy!)
    })
    ...
    it('helperText가 있을 때 aria-describedby가 설정된다', () => {
      ...
      const describedBy = textarea.getAttribute('aria-describedby')
      expect(describedBy).toBeTruthy()
      expect(screen.getByText('최대 500자까지 입력 가능합니다')).toHaveAttribute('id', describedBy!)
    })
  ...
  describe('엣지 케이스 테스트', () => {
    ...
    it('value와 onChange가 동시에 제공되면 제어 컴포넌트로 동작한다', () => {
      const ControlledTextArea = () => {
        ...
        return <TextArea label="설명" value={value} onChange={(e) => setValue(e.target.value)} />
      }
